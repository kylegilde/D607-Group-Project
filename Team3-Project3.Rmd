...
This challenging project for DATA607 focuses on the team building, collaboration and leadership required to succeed in the field of data science and analytics. We are instructed to work closely in groups toward the task of answering a specific question: **"Which are the most valued data science skills?"** Our team quickly established a rapport and communications channel on #slack and set to the task with gusto and shared responsibility. We chose as our leader Duubar Villalobos Jimenez, and under his coordination we assigned the variety of tasks. Of course, data is required. We chose to collect data from the website Paysa, which lists a wide variety of tech postings, the skills associated with each posting and several salary components. Details follow, but our analysis shows the Machine Learning is the skill that was most-requested in our sample of 390 job listings, while the highest-valued skill measured by mean compensation was Strategy - reflecting the higher pay offered for management, leadership and vision in this rapidly evolving profession.
...
We agreed to use **Slack** as our Team Collaboration platform. From Slack we were able to perform live meetups by using "join.me" permitting screen sharing, this allowing some presentations in order to get updated in some specific topics.
...
This code use string handling to separate the text into an initial set of columns.
...
Next, we stored this initial table on a remote MySQL server. We provided clear instructions for all team members about how to read tables into R. 
...
The below is our tidy table with a total of `r nskills` skills. (In a subsequent step, we collapse repetitive skills (i.e., "C" and "C++'.)
...
Another way of Analysing the above data is by performing a tree analysis. This tree shows Expected Salary for jobs associated with Machine Learning skills in Washington and California.

Following are the highest-paid jobs in the data.

```{r Raghu_Analysis0, echo=FALSE, warning=FALSE, error=FALSE, cache=FALSE, message=FALSE}
...
p <- plot_ly(data = g1, x = ~paste(Company, "#", Rank), y = ~Salary, text = ~paste("Position:", Position), type = 'bar') %>%
  ...
  layout(showlegend = FALSE, 
         title = paste("Top ", top, " Offered Expected Salaries by Employers"), 
         autosize = F, width = 750, height = 500
         )
...
The below combination of desired skills by employers will generate the top Expected Salary of \$`r format(topPaidSkills_ExpectedSalary$Salary[1], scientific=FALSE)`. 

```{r, echo=FALSE, warning=FALSE, error=FALSE, message=FALSE}

kable(topPaidSkills_ExpectedSalary1, format = "html", caption = paste("Table: Top paid skills by top paid Expected Salaries"))

```


```{r Raghu_Tree1, echo=FALSE, warning=FALSE, error=FALSE, message=FALSE}
...
p <- plot_ly(data = g1, x = ~paste(Company, "#", Rank), y = ~Salary, text = ~paste("Position:", Position), type = 'bar') %>%
  layout(xaxis = list(title = ""),
         ...
         margin = list(b = 150, l = 50, r = 70)) %>%
  layout(showlegend = FALSE, 
         title = paste("Top ", top, " Offered Signing Salaries by Employers"), 
         autosize = F, width = 750, height = 500
         )
...
The below combination of desired skills by employers will generate the top Signing Salary of \$`r format(topPaidSkills_SigningSalary$Salary[1], scientific=FALSE)`.  

```{r, echo=FALSE, warning=FALSE, error=FALSE, message=FALSE}

kable(topPaidSkills_SigningSalary1, format = "html", caption = paste("Table: Top paid skills by top paid Signing Salaries"))
...
The below combination of desired skills by employers will generate the top Annual Salary of \$`r format(topPaidSkills_AnnualSalary$Salary[1], scientific=FALSE)`. 

```{r, echo=FALSE, warning=FALSE, error=FALSE, message=FALSE}

kable(topPaidSkills_AnnualSalary1, format = "html", caption = paste("Table: Top paid skills by top paid Annual Salaries"))
...
names(topPaidSkills_ExpectedSalary1) <- c("Skills", "Expected Salary")
names(topPaidSkills_SigningSalary1) <- c("Skills", "Signing Salary")
names(topPaidSkills_AnnualSalary1) <- c("Skills", "Annual Salary")
...
The below table displays a top combination of skills in order to obtain a high demand skills job with high pay.
...
From the above table we can deduct the best combination of skills in order to obtain a high paid job with high demand of top skills.
...
## Highest-Valued Skills by Mean Compensation

Some of the highest-valued skills are not the most common technical skills. They include Strategy, Leadership, Management and Data Science, a catch-all. ETL -- for Extract, Transfer and Load -- is a critical area of data warehousing.
...
## Relative Value of Skills in Expected Salary
...
# fit ANOVA
fit <- lm(skill_val ~ new_skill.x, contrasts = "contr.sum", data = tom_df2)
